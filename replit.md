...
-   **Left-Pane / DocViewer Standard (Standardization Pass applied — all scan tools)**:
    All scan-tool document viewers must follow this exact header structure:
    - **Row 1 (h-7, tool identity)**: Tool-icon (dimmed) + tool name (`text-[10px] text-white/28 font-medium`) + status badge on right (color-coded: emerald = good, amber = caution/partial, red = risk). `bg-white/[0.01]` background, `border-white/[0.04]` border.
    - **Row 2 (h-9, file toolbar)**: FileText icon (tool color, dimmed) + filename (`text-xs text-white/45 flex-1 truncate`) + section count (`text-xs text-white/18`) + divider + zoom buttons [Fit / 75% / 100%].
    - **Section cards**: Dark absolute colors — `bg-white/[0.015] border-white/[0.05]`; active card: `bg-violet-500/[0.06] border-violet-500/45 ring-violet-500/20`; dimmed (other section active): `opacity-40`.
    - **Footer section nav** (h-10): section label or "N of M" + numbered jump buttons (1–8, active = violet fill) + "Jump to section" label on right.
    - **Evidence banner**: `border-violet-500/28 bg-violet-500/[0.07]` with pulsing dot + source title + snippet (italic) + dismiss X. ContractReview uses amber tint. Injected as `mx-3 mt-2 mb-1 shrink-0` between toolbar and section body.
    - **Width**: single-split tools use exactly `w-[58%]`.
    - **DocViewer outer bg**: `bg-[#0d0d10]`.
    - Tools conforming to this standard: AnalyzeDocument (benchmark), TrustCheck, ContractReview, ClauseExtractor. Compare Versions uses three 33/33/34 columns each with the same two-row header (tool name + section count on row 1; file label + filename + change badge on row 2). Redact is APPROVED AND LOCKED — light paper theme, do not change.
    - **Processing state**: Already standardized via shared `DocumentScanScreen` component (all tools except Redact and ContractBuilder use it).
-   **Analyze a Document (refined A-G action-plan layout — `/analyze` + `/analyze-document`)**: `/analyze` → `AnalyzePage.tsx` handles the full upload + processing + error flow (do not change). On success, context is set and user is redirected to `/analyze-document` → `AnalyzeDocument.tsx` (results). `AnalyzeDocument.tsx` implements the premium split-screen results page: desktop — document viewer (58% width) on left showing extracted `analysis.sections` as scrollable section cards; intelligence panel (42%, `flex-1`) on right with strict A–G section order: **A. Plain-English Summary** (violet card) → **B. Action Plan Snapshot** (compact chip row: X actions, X urgent, X missing, X deadlines, X source-backed + overall status badge: Ready / Needs completion / Needs review) → **C. Step-by-Step Required Actions** (numbered step cards, each showing: step#, title, priority badge [Urgent/Important/Optional], clickable status dot [not-started→in-progress→complete, local/session state], category/responsible party, plain-English instruction, source chip) → **D. Missing Items / Information Needed** (amber border, from `requiredDocuments.filter(!obtained)`, each card shows name, description, "appears missing" wording, source chip) → **E. Deadlines & Time-Sensitive Items** (deadline cards with date, title, consequence via description, hard/soft flag, source chip) → **F. Key Risks / Watchouts** (secondary, slim cards, source chip) → **G. Source Traceability** (violet border, lists action steps + deadlines + risks with source chips) → Key Terms (after G, gated by isPro) → Follow-up Tools. Source chips: `findBestSection()` word-overlap matches evidence to best `DocumentSection`, scrolls it into view, applies violet highlight ring + evidence banner, and highlights the related action card with violet ring. Action status is local `useState<Record<string, StepStatus>>` initialized from `step.completed`, cycles not-started→in-progress→complete on dot click. Low-confidence state (`overallConfidence === "low"`): LowConfPanel shows amber warning banner + partial summary + numbered partial action steps (marked `~`) + partial risks + recommended next steps. Mobile: tab layout defaulting to Analysis tab (Summary → Snapshot → Steps → Missing → Deadlines → Risks → Source chips); chip clicks switch to Document tab. Safe wording enforced: "appears to require", "verify before acting", "based on the document", "not legal advice". Key files: `artifacts/plainpath/src/pages/AnalyzePage.tsx` (do not change), `artifacts/plainpath/src/pages/AnalyzeDocument.tsx`, `App.tsx`.
