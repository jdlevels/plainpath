...
- Confirmed on 2026-04-27: `allowlistEnforcement()` now fails closed when `ALLOWED_EMAILS` is empty. Clerk-authenticated routes should be treated as allowlist-gated in production, while unauthenticated routes remain public unless the handler enforces auth itself.
- Confirmed on 2026-04-27: subscriber ownership and plan resolution still fall back to mutable email matches after a record has been linked to a Clerk user ID. Future scans should keep treating `billingDb.ts`, `resolvePlan.ts`, `entitlements.ts`, and Stripe billing-portal flows as a single identity-binding surface.
...
- Confirmed on 2026-04-27: `/api/documents/redact-pdf` remains a synchronous single-process heavy-compute path with attacker-influenced work factors. Future scans should look for caps on redact term count, match attempts, and worker isolation or timeouts.
...
- Confirmed on 2026-04-27: the public demo quota still relies on `User-Agent` plus raw `X-Forwarded-For` fingerprinting. Future abuse reviews should treat anonymous demo identity as attacker-controlled until usage is bound to trusted proxy-derived client IPs or stronger server-issued identities.
