    ...
    <div className="min-h-screen bg-[#F8F7F4] pb-24">
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6">
        ...
        <Tabs.Root value={activeTab} onValueChange={setActiveTab}>
          ...
          {/* ── Content pane ────────────────────────────── */}
          <div className="bg-white rounded-3xl border border-border/30 shadow-lg shadow-black/[0.04] overflow-hidden min-h-[540px]">
  ...
  return (
    <button
      ...
      className={`flex items-center gap-2 px-3.5 py-2 rounded-xl text-sm font-semibold border transition-all hover:shadow-sm ${
        warn && value > 0
          ? "bg-red-50 border-red-200/60 text-red-700 hover:border-red-300"
          : "bg-white border-border/50 text-foreground hover:border-primary/30"
      }`}
    >
      <span className="text-lg font-bold font-display tabular-nums">{value}</span>
      <span className={`text-xs font-medium ${warn && value > 0 ? "text-red-600/70" : "text-muted-foreground"}`}>{label}</span>
    </button>
  )
}
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          ...
          <button key={tab} onClick={() => onTabChange(tab)} className="text-left group">
            <div className={`p-4 rounded-2xl border transition-all group-hover:shadow-md ${warn ? "bg-red-50/60 border-red-200/50 group-hover:border-red-300" : "bg-secondary/30 border-transparent group-hover:border-primary/20"}`}>
              <p className={`text-xs font-semibold uppercase tracking-wide mb-2 ${warn ? "text-red-600/70" : "text-muted-foreground"}`}>{label}</p>
              <p className={`text-3xl font-display font-bold ${warn ? "text-red-600" : "text-foreground"}`}>{value}</p>
            </div>
          </button>
        ))}
      </div>
        ...
        <div>
          ...
          <div className="space-y-2">
            {highPriority.slice(0, 3).map(step => (
              <div key={step.id} className="flex items-center gap-3 p-3 rounded-xl bg-amber-50 border border-amber-200/50">
            ...
            {highRisks.slice(0, 2).map(risk => (
              <div key={risk.id} className="flex items-center gap-3 p-3 rounded-xl bg-red-50 border border-red-200/50">
    ...
    <div className="space-y-8 max-w-3xl">
      ...
      {hardDls.length > 0 && (
        <MissingSection title="Hard deadlines" icon={<Calendar className="w-3.5 h-3.5" />}>
          <div className="space-y-2">
            {hardDls.map(dl => (
              <div key={dl.id} className="flex gap-3 p-3.5 rounded-xl bg-red-50 border border-red-200/50">
                <Clock className="w-4 h-4 text-red-500 mt-0.5 shrink-0" />
                <div>
                  <p className="font-bold text-sm text-foreground">{dl.title}</p>
                  <p className="text-xs font-semibold text-red-600 mt-0.5">{dl.date}</p>
      ...
      {highRisks.length > 0 && (
        <MissingSection title="High-severity risks" icon={<AlertTriangle className="w-3.5 h-3.5" />}>
          <div className="space-y-2">
            {highRisks.map(risk => (
              <div key={risk.id} className="p-3.5 rounded-xl bg-red-50 border border-red-200/50">
              ...
              {analysis.deadlines.map((dl) => (
                <div key={dl.id} className="flex gap-4">
                  <div className={`w-7 h-7 rounded-full border-2 flex items-center justify-center shrink-0 mt-2 z-10 ${
                    dl.isHard ? "bg-red-50 border-red-400" : "bg-white border-border"
                  }`}>
                    <div className={`w-2.5 h-2.5 rounded-full ${dl.isHard ? "bg-red-400" : "bg-muted-foreground/40"}`} />
                  </div>
                  <div className={`flex-1 p-4 rounded-2xl border mb-1 ${dl.isHard ? "bg-red-50/60 border-red-200/60" : "bg-white border-border/50 shadow-sm"}`}>
                    <div className="flex items-start justify-between gap-3 mb-1">
                      <div>
                        <p className={`text-sm font-bold ${dl.isHard ? "text-red-700" : "text-primary"}`}>{dl.date}</p>
    ...
    <div className="space-y-10">
      ...
      {analysis.risks.length === 0
        ...
        : <div className="space-y-3">
            {analysis.risks.map((risk) => {
              ...
              return (
                <div key={risk.id} className={`rounded-2xl border p-5 ${isHigh ? "bg-red-50/60 border-red-200/50" : "bg-white border-border/50 shadow-sm"}`}>
                  <div className="flex items-start justify-between gap-3 mb-2">
                    <div className="flex items-start gap-3">
                      <div className={`w-8 h-8 rounded-xl flex items-center justify-center shrink-0 mt-0.5 ${isHigh ? "bg-red-100" : "bg-secondary"}`}>
                        <AlertTriangle className={`w-4 h-4 ${isHigh ? "text-red-600" : "text-muted-foreground"}`} />
                      </div>
                      <div>
                    ...
                    <span className={`inline-flex px-2.5 py-1 rounded-full border text-[10px] font-bold uppercase tracking-wide shrink-0 ${
                      isHigh ? "bg-red-100 border-red-200 text-red-700"
                      : risk.severity === "medium" ? "bg-amber-50 border-amber-200 text-amber-700"
      ...
      {analysis.followUpQuestions.length > 0 && (
        <div>
          ...
          <div className="space-y-2.5">
            {analysis.followUpQuestions.map(q => (
              <div key={q.id} className="flex gap-3 p-4 rounded-xl border border-amber-200/50 bg-amber-50/40">
                <AlertCircle className="w-4 h-4 text-amber-500 mt-0.5 shrink-0" />
                <div>
  ...
  return (
    <div className={`flex items-start gap-3 rounded-xl border transition-all ${compact ? "p-3" : "p-4"} ${
      doc.obtained
        ...
        : "bg-white border-border/40 hover:shadow-sm hover:border-primary/20"
    }`}>
      <div className="shrink-0 mt-0.5">
