export default function ExamplesSlide() {
  const examples = [
    ...
    },
    {
      ...
      highlight: "\"All inventions conceived during employment\" clause assigns personal side-project IP to employer. Negotiation language included.",
      ...
      <div className="relative flex h-full flex-col px-[8vw] py-[6vh]">
        <div className="mb-[3vh]">
          ...
          <h2 className="text-[3.8vw] font-extrabold leading-[1.05] tracking-tight text-[#f0f4f8] font-display">
            One tool per scenario. Both live at plainpathapp.com.
          </h2>
        </div>

        <div className="grid grid-cols-2 gap-[2.5vw] flex-1 content-center max-w-[75vw] mx-auto w-full">
          {examples.map((ex) => (
            <div key={ex.tool} className="bg-[#0f1929] rounded-[1.3vw] p-[2.2vw] flex flex-col gap-[1.6vh]" style={{ border: `1px solid ${ex.toolBorder}` }}>
              ...
              <div>
                <div className="text-[1.5vw] font-bold text-[#f0f4f8] font-display leading-snug mb-[0.6vh]">{ex.title}</div>
                <div className="text-[1.15vw] text-[#6b7a8d] font-body leading-snug">{ex.desc}</div>
              </div>
              <div className="h-px bg-[#1e2d44]" />
              <div className="grid grid-cols-3 gap-[0.6vw]">
                {ex.stats.map((s) => (
                  <div key={s.l} className="bg-[#172035] rounded-[0.5vw] p-[0.9vw] text-center">
                    <div className="text-[1.8vw] font-extrabold font-display" style={{ color: ex.toolColor }}>{s.v}</div>
                    <div className="text-[0.85vw] text-[#6b7a8d] font-body mt-[0.15vh]">{s.l}</div>
                  </div>
                ))}
              </div>
              <div className="rounded-[0.7vw] p-[1.2vw] flex-1" style={{ background: ex.toolBg }}>
                <div className="text-[0.9vw] font-semibold font-body mb-[0.4vh]" style={{ color: ex.toolColor }}>{ex.highlightLabel}</div>
                <div className="text-[1.05vw] text-[#f0f4f8]/80 font-body leading-snug">"{ex.highlight}"</div>
              </div>
            </div>
          ))}
        </div>
