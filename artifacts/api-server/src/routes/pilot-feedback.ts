...
router.post("/pilot-feedback", async (req, res) => {
  ...
  try {
    const id = await savePilotFeedback({
    ...
    console.error("Failed to save pilot feedback:", err)
    return res.status(500).json({ error: "Failed to save feedback" })
  }
})

router.get("/pilot-feedback/summary", async (_req, res) => {
  try {
    const summary = await getPilotFeedbackSummary()
    return res.json(summary)
  } catch (err) {
    console.error("Failed to get pilot feedback summary:", err)
    return res.status(500).json({ error: "Failed to load summary" })
  }
})

router.get("/pilot-feedback/report", async (_req, res) => {
  try {
    const records = await listPilotFeedback()
    const summary = await getPilotFeedbackSummary()
    ...
    console.error("Failed to generate pilot report:", err)
    return res.status(500).json({ error: "Failed to generate report" })
  }
})

router.get("/pilot-feedback", async (_req, res) => {
  try {
    const records = await listPilotFeedback()
    return res.json({ records })
  } catch (err) {
    console.error("Failed to list pilot feedback:", err)
