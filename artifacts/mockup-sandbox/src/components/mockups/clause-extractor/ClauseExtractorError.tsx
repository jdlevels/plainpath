import { FileText, AlertCircle, RefreshCcw, Upload, MessageCircle, FileSearch, RotateCcw } from "lucide-react";
      ...
      <div className="h-12 border-b border-white/[0.06] flex items-center px-5 gap-2.5 shrink-0">
        ...
        <div className="ml-auto">
          <div className="h-6 px-2.5 rounded-full border border-white/[0.08] flex items-center gap-1.5">
            ...
            <span className="text-[11px] text-white/35">Clause extraction could not be completed</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center p-8">
        <div className="max-w-sm w-full">
          ...
          <div className="rounded-2xl border border-white/[0.08] bg-white/[0.03] p-8 text-center mb-6">
            ...
            <h2 className="text-base font-bold text-white/80 mb-2">Clause extraction could not be completed.</h2>
            <p className="text-xs text-white/38 leading-relaxed mb-6">
              The file appears to be password-protected or encrypted. PlainPath needs readable document text to identify clauses, obligations, and terms to verify.
            </p>

            {/* File chip */}
            <div className="rounded-xl border border-white/[0.07] bg-white/[0.02] px-4 py-3 flex items-center gap-3 text-left mb-6">
              ...
              <div>
                <p className="text-xs font-medium text-white/55">Partnership_Agreement_Protected.pdf</p>
            ...
            <div className="flex gap-2.5">
              ...
              <button className="flex-1 border border-white/[0.1] text-white/55 text-xs font-medium py-2.5 rounded-xl hover:bg-white/[0.04] transition-colors flex items-center justify-center gap-1.5">
          ...
          <div>
            ...
            <div className="space-y-2">
              {[
                ...
                { icon: <FileSearch className="w-3.5 h-3.5 text-white/35" />, title: "Analyze a Document", desc: "Full plain-English analysis once the file is readable." },
              ].map(opt => (
                <button key={opt.title} className="w-full rounded-xl border border-white/[0.07] bg-white/[0.02] hover:bg-white/[0.04] transition-colors px-4 py-3 flex items-center gap-3 text-left">
                  ...
                  <RefreshCcw className="w-3 h-3 text-white/15 shrink-0" />
                </button>
              ))}
            </div>
          </div>
