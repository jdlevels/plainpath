...
const STEPS = [
  ...
  { label: "Identifying clause headings",           desc: "Finding article headings and numbered clauses…",        done: true,  active: false },
  ...
  { label: "Building extraction summary",           desc: "Organizing everything into structured output…",        done: false, active: false },
];
      ...
      <div className="flex-1 flex min-h-0">
        ...
        <div className="w-[57%] border-r border-white/[0.05] flex flex-col overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-4">
            {[
              { num: 1, label: "Read",        active: false, done: true  },
              { num: 2, label: "Read",        active: false, done: true  },
              { num: 3, label: "Extracting…", active: true,  done: false },
            ].map(page => (
              <div key={page.num} className={`rounded-xl border p-4 ${page.active ? "border-violet-500/40 bg-violet-500/[0.04] ring-1 ring-violet-500/20" : "border-white/[0.06] bg-white/[0.015]"}`}>
                <div className="flex items-center justify-between mb-3">
                  ...
                  {page.active
                    ...
                    : <span className="text-[10px] text-emerald-400/50">read</span>
                  }
                </div>
                <div className="space-y-2">
                  {[80, 60, 75, 45, 70, 55].map((w, i) => (
        ...
        <div className="flex-1 flex flex-col overflow-y-auto p-6 gap-5">
          ...
          <div className="rounded-xl border border-white/[0.07] bg-white/[0.02] p-4">
            ...
            <div className="h-1 rounded-full bg-white/[0.06] overflow-hidden mb-2">
              <div className="h-full rounded-full bg-violet-500" style={{ width: "42%" }} />
            </div>
            ...
            {STEPS.map((step, i) => (
              <div key={i} className={`flex items-start gap-3 px-4 py-3.5 ${step.active ? "bg-violet-500/[0.05]" : ""}`}>
                ...
                <div>
                  ...
                  {(step.done || step.active) && <p className={`text-[10px] mt-0.5 ${step.done ? "text-white/22" : "text-white/38"}`}>{step.desc}</p>}
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="rounded-xl border border-white/[0.06] bg-white/[0.01] p-4">
            ...
            <div className="grid grid-cols-2 gap-1.5">
              {["Clauses by category","Obligations & owners","Dates & deadlines","Plain-English summary","Missing clause flags","Source-backed chips"].map(f => (
