...
// Slice 6 (T001–T004): Audit export, PDF Editor handoff (formal), session management.
// ─────────────────────────────────────────────────────────────────────────────
...
// Slice 6 — Formal handoff to PDF Editor.
//
// HANDOFF CONTRACT
// ─────────────────────────────────────────────────────────────────
// Request (body):
//   { diffIds?: string[], mode?: "all" | "selected" }
//   - omit diffIds → mode "all"   → every diff item with rect_revised is highlighted
//   - include diffIds → mode "selected" → only the listed diff IDs are highlighted
//
// Response:
//   { handoffId: string, pdfEditorSessionId: string, highlightCount: number }
//
// Storage:
//   1. A new row is inserted into cv_handoff_records recording:
//      - cv_session_id, pdf_editor_session_id, user_id, mode,
//        selected_diff_ids (JSONB), highlight_count
//   2. A new row is inserted into pdf_editor_sessions carrying:
//      - the revised PDF bytes (from storage)
//      - an initial `ops` JSONB array of highlight overlays, one per diff item:
//          { id: "cv-{diffId}", kind: "highlight", pageIndex, x, y, w, h, highlightColor, opacity }
//        pageIndex = max(0, item.page_revised - 1)  (engine is 1-indexed, editor is 0-indexed)
//        highlightColor: high=#fca5a5, medium=#fcd34d, low=#6ee7b7
//        opacity: 0.35
//
// Resolution:
//   - PDF Editor navigates to /pdf-editor/:pdfEditorSessionId?fromCompare=1&handoffId=:handoffId
//   - ?fromCompare=1 controls the contextual banner in PdfEditorSession.tsx
//   - handoffId is stored for provenance; PDF Editor does NOT re-fetch it for rendering
//   - All rendering state is fully baked into pdf_editor_sessions.ops at handoff time
//
// Filtering:
//   - Only items with rect_revised + page_revised present are included as highlights
//   - Items without location data are omitted (they can't be highlighted spatially)
//
// Compare Versions remains read-only: the CV session is not modified by the handoff.

router.post("/sessions/:id/handoff", requireAuth, async (req: any, res: any) => {
  try {
    const cvResult = await pool.query(
      `SELECT id, title, revised_storage_key, revised_file_name, diff_result, status
       FROM compare_versions_sessions
       WHERE id = $1 AND user_id = $2 AND deleted_at IS NULL`,
      [req.params.id, req.userId],
    );
    if (!cvResult.rows.length) return res.status(404).json({ error: "not_found" });
    ...
    // Determine mode and diff selection
    const requestedDiffIds: string[] | undefined = Array.isArray(req.body?.diffIds)
      ? req.body.diffIds
      : undefined;
    const mode: "all" | "selected" = requestedDiffIds ? "selected" : "all";

    // Download revised PDF
    const revisedBuf = await downloadPdf(cvRow.revised_storage_key);

    // Build highlight ops from diff_result
    const diffResult = cvRow.diff_result as any;
    const allItems: any[] = diffResult?.items ?? [];

    const candidateItems = mode === "selected"
      ? allItems.filter((item: any) => requestedDiffIds!.includes(item.id))
      : allItems;
    ...
    const handoffDiffIds: string[] = [];

    for (const item of candidateItems) {
      if (item.rect_revised && item.page_revised != null) {
        const pageIndex = Math.max(0, item.page_revised - 1);
        initialOps.push({
          id: `cv-${item.id}`,
          kind: "highlight",
          pageIndex,
        ...
        handoffDiffIds.push(item.id);
      }
    }

    // Create pdf_editor_sessions row with revised PDF
    ...
    await pool.query(
      ...
      [
        pdfEditorSessionId, req.userId,
        cvRow.revised_file_name || "revised.pdf",
        revisedBuf.length, pdfBytesValue, pdfStorageKey,
        JSON.stringify(initialOps),
      ],
    );

    // Create formal handoff record linking CV session → PDF Editor session
    const handoffResult = await pool.query(
      `INSERT INTO cv_handoff_records
         (cv_session_id, pdf_editor_session_id, user_id, mode, selected_diff_ids, highlight_count)
       VALUES ($1, $2, $3, $4, $5::jsonb, $6)
       RETURNING id`,
      [
        req.params.id, pdfEditorSessionId, req.userId,
        mode, JSON.stringify(handoffDiffIds), initialOps.length,
      ],
    );
    const handoffId: string = handoffResult.rows[0].id;

    console.log(
      `[compare-versions] handoff ${handoffId}: cv=${req.params.id} → pdf-editor=${pdfEditorSessionId} ` +
      `mode=${mode} highlights=${initialOps.length}`,
    );

    return res.status(201).json({ handoffId, pdfEditorSessionId, highlightCount: initialOps.length });
  } catch (err: any) {
...
// ─── GET /api/compare-versions/handoffs/:handoffId ───────────────────────────
// Resolve a handoff record by ID (provenance lookup, not required for rendering).

router.get("/handoffs/:handoffId", requireAuth, async (req: any, res: any) => {
  try {
    const result = await pool.query(
      `SELECT id, cv_session_id, pdf_editor_session_id, mode, selected_diff_ids, highlight_count, created_at
       FROM cv_handoff_records
       WHERE id = $1 AND user_id = $2`,
      [req.params.handoffId, req.userId],
    );
    if (!result.rows.length) return res.status(404).json({ error: "not_found" });
    const row = result.rows[0];
    return res.json({
      id: row.id,
      cvSessionId: row.cv_session_id,
      pdfEditorSessionId: row.pdf_editor_session_id,
      mode: row.mode,
      selectedDiffIds: row.selected_diff_ids ?? [],
      highlightCount: row.highlight_count,
      createdAt: row.created_at,
    });
  } catch (err) {
    console.error("[compare-versions] get handoff error", err);
    return res.status(500).json({ error: "server_error" });
  }
});
