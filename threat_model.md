...
- Confirmed on 2026-04-27: server-side paid-feature enforcement remains an active risk area. `resolvePlan.ts` currently defaults non-subscribers to `starter`, so future scans should continue auditing whether Starter is intentionally free or whether direct API access can bypass marketed subscription gates. `/api/documents/chat`, `/api/documents/redact-pdf`, and the `/api/builder/*` routes remain useful entitlement-audit anchors because client-side plan gating is not authoritative.
