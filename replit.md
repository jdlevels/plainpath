...
-   **Compare Versions**: PDF-first workspace for comparing original and revised documents. Slices 1–4 shipped. Engine: Stage A spatial/visual diff, Stage B Myers text diff, Stage C structural signals. Slice 4 adds: union-rect group zones per pane (iterative overlap merging, stable IDs, count badge on multi-item groups), violet selection ring + hover brightness sync bidirectionally across both panes and summary, MiniPopover for multi-item group clicks, SummaryPanel with severity filter tabs + page dropdown filter + sort (HIGH→MED→LOW then page order), severity override dropdown per row (debounced PATCH /review persist, overridden badge), linked-note indicator per row, full notes CRUD in NotesRail (add/delete/resolve/link-to-diff for both structured notes and watchlist), resolved rows visually de-emphasized. Backend: `PATCH /sessions/:id/review` for diff_result override persistence. Key files: `compareVersionsGrouping.ts`, `compareVersionsTypes.ts`, `CompareVersionsSession.tsx`.
