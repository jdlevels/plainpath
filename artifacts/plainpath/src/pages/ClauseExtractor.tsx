...
function ClauseCard({
  ...
  clauseKey: string
  clause: ClausePresence
  selected?: boolean
  onSelect?: () => void
}) {
  return (
    <div
      role={clause.present ? "button" : undefined}
      tabIndex={clause.present ? 0 : undefined}
      onClick={clause.present ? onSelect : undefined}
      onKeyDown={clause.present ? (e) => { if (e.key === "Enter" || e.key === " ") onSelect?.() } : undefined}
      className={`rounded-xl border p-4 transition-all ${clause.present ? "cursor-pointer" : ""} ${
        selected
          ? "border-violet-400 dark:border-violet-500 bg-violet-50/90 dark:bg-violet-950/40 ring-1 ring-violet-300/60 dark:ring-violet-700/60"
          : clause.present
          ? "border-emerald-200 dark:border-emerald-700 bg-emerald-50/80 dark:bg-emerald-950/30 hover:border-emerald-300 dark:hover:border-emerald-600"
          : "border-border/60 bg-muted/40"
      }`}
    >
      <div className="flex items-start gap-2.5">
        {clause.present
          ? <CheckCircle2 className={`w-4 h-4 shrink-0 mt-0.5 ${selected ? "text-violet-500" : "text-emerald-500"}`} />
          : <XCircle className="w-4 h-4 text-muted-foreground/40 shrink-0 mt-0.5" />}
        <div className="flex-1 min-w-0">
          <p className={`text-xs font-semibold leading-tight ${clause.present ? "text-foreground" : "text-muted-foreground/70"}`}>
            {CLAUSE_LABELS[clauseKey] ?? clauseKey}
          </p>
          {clause.present && clause.summary && (
            <p className="text-[11px] text-muted-foreground mt-1.5 leading-relaxed">{clause.summary}</p>
          )}
          {selected && clause.snippet && (
            <div className="mt-3 border-l-2 border-violet-400 pl-2.5">
              <p className="text-[10px] uppercase tracking-widest font-semibold text-violet-500/80 mb-1">Source excerpt</p>
              <p className="text-[11px] text-muted-foreground italic leading-relaxed">"{clause.snippet}"</p>
            </div>
          )}
        </div>
      </div>
...
/* ─── Filter types ──────────────────────────────────────────── */
type FilterKey = "all" | "payment" | "termination" | "obligations" | "liability" | "confidentiality" | "deadlines" | "missing"
...
/* ─── Missing clause card ───────────────────────────────────── */
function MissingClauseCard({ field }: { field: string }) {
  return (
    <div className="rounded-xl border border-amber-200/80 dark:border-amber-800/50 bg-amber-50/50 dark:bg-amber-950/20 p-4">
      <div className="flex items-start gap-3">
        <AlertTriangle className="w-4 h-4 text-amber-500 shrink-0 mt-0.5" />
        <div className="flex-1 min-w-0">
          <p className="text-xs font-semibold text-foreground leading-tight">{field}</p>
          <p className="text-[11px] text-muted-foreground mt-1">
            Status: <span className="font-medium text-amber-600 dark:text-amber-400">Not identified</span>
          </p>
          <p className="text-[11px] text-muted-foreground mt-1.5 leading-relaxed">
            Expected but not found in the document. Verify whether it appears under a different label, or ask the other party to clarify.
          </p>
          <p className="text-[10px] text-muted-foreground/50 mt-2 flex items-center gap-1">
            <XCircle className="w-3 h-3" /> No exact source found
          </p>
        </div>
      </div>
    </div>
...
function ResultsPanelContent({
  ...
  const dateCount = Object.values(r.keyDates).filter(Boolean).length

  const clauseFilterKeys: Partial<Record<FilterKey, string[]>> = {
    termination: ["terminationClause", "autoRenewal"],
    liability: ["liabilityCap", "indemnity"],
    confidentiality: ["confidentiality"],
  }

  const visibleClauseKeys = clauseFilterKeys[activeFilter] ?? null
  const filteredClauses = visibleClauseKeys
    ? legalClauses.filter(([k]) => visibleClauseKeys.includes(k))
    : legalClauses

  const showDates      = activeFilter === "all" || activeFilter === "deadlines"
  const showParties    = activeFilter === "all"
  const showFinancial  = activeFilter === "all" || activeFilter === "payment"
  const showClauses    = activeFilter === "all" || ["termination", "liability", "confidentiality"].includes(activeFilter)
  const showObligations = activeFilter === "all" || activeFilter === "obligations"
  const showMissing    = activeFilter === "all" || activeFilter === "missing"

  function handleClauseSelect(key: string) {
    const next = selectedClauseKey === key ? null : key
    setSelectedClauseKey(next)
    if (next) onScrollToDocument?.()
  }

  const selectedClause = selectedClauseKey
    ? (r.legalClauses[selectedClauseKey as keyof typeof r.legalClauses] as ClausePresence | undefined)
    : null
  ...
  return (
    <div className="h-full flex flex-col bg-background overflow-hidden">
      ...
      {/* Summary stats chips */}
      <div className="shrink-0 px-4 py-2.5 border-b border-border/40 flex items-center gap-2 overflow-x-auto" style={{ scrollbarWidth: "none" }}>
        {[
          { label: "Found", count: presentCount, cls: "text-emerald-700 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/40 border-emerald-200 dark:border-emerald-800/60" },
          { label: "Missing", count: r.missingFields.length, cls: "text-amber-700 dark:text-amber-400 bg-amber-50 dark:bg-amber-950/40 border-amber-200 dark:border-amber-800/60" },
          { label: "Obligations", count: r.obligations.length, cls: "text-purple-700 dark:text-purple-400 bg-purple-50 dark:bg-purple-950/40 border-purple-200 dark:border-purple-800/60" },
          { label: "Dates", count: dateCount, cls: "text-blue-700 dark:text-blue-400 bg-blue-50 dark:bg-blue-950/40 border-blue-200 dark:border-blue-800/60" },
          { label: "Parties", count: r.parties.length, cls: "text-violet-700 dark:text-violet-400 bg-violet-50 dark:bg-violet-950/40 border-violet-200 dark:border-violet-800/60" },
        ].map(({ label, count, cls }) => (
          <div key={label} className={`shrink-0 flex items-center gap-1.5 border rounded-full px-2.5 py-1 ${cls}`}>
      ...
      {/* Filter chips */}
      <div className="shrink-0 px-4 py-2 border-b border-border/40 flex items-center gap-1.5 overflow-x-auto" style={{ scrollbarWidth: "none" }}>
        {FILTERS.map(({ key, label }) => (
          <button
            key={key}
            type="button"
            onClick={() => { setActiveFilter(key); setSelectedClauseKey(null) }}
            className={`shrink-0 text-[11px] font-medium px-3 py-1.5 rounded-full border transition-colors whitespace-nowrap ${
              activeFilter === key
                ? "bg-violet-600 text-white border-violet-600"
                : "bg-background text-muted-foreground border-border/60 hover:border-violet-400/60 hover:text-foreground"
            }`}
          >
            {label}
          </button>
      ...
      {/* Source banner — when a clause is selected */}
      {selectedClauseKey && selectedClause?.present && (
        <div className="rounded-xl border border-violet-200 dark:border-violet-800 bg-violet-50/80 dark:bg-violet-950/30 px-4 py-3">
          <p className="text-[10px] font-bold uppercase tracking-widest text-violet-500 mb-1">Source used for this clause</p>
          {selectedClause.snippet ? (
            <p className="text-[11px] text-muted-foreground italic leading-relaxed">"{selectedClause.snippet}"</p>
          ) : (
            <p className="text-[11px] text-muted-foreground">Section identified in document analysis.</p>
          )}
        </div>
      )}

      {/* Key Dates */}
      {showDates && Object.values(r.keyDates).some(Boolean) && (
      ...
      {/* Parties */}
      {showParties && r.parties.length > 0 && (
        <Card className="p-5 rounded-2xl border border-border/50">
      ...
      {/* Financial Terms */}
      {showFinancial && (r.financialTerms.paymentAmount || r.financialTerms.paymentSchedule || r.financialTerms.lateFees || r.financialTerms.refundLanguage || r.financialTerms.otherTerms.length > 0) && (
        <Card className="p-5 rounded-2xl border border-border/50">
      ...
      {/* Legal Clauses */}
      {showClauses && filteredClauses.length > 0 && (
        <Card className="p-5 rounded-2xl border border-border/50">
          ...
          <div className="grid grid-cols-1 gap-2.5">
            {filteredClauses.map(([key, clause]) => (
              <ClauseCard
                key={key}
                clauseKey={key}
                clause={clause}
                selected={selectedClauseKey === key}
                onSelect={() => handleClauseSelect(key)}
              />
            ))}
          </div>
        </Card>
      ...
      {/* Obligations */}
      {showObligations && r.obligations.length > 0 && (
        <Card className="p-5 rounded-2xl border border-border/50">
      ...
      {showMissing && r.missingFields.length > 0 && (
        <Card className="p-5 rounded-2xl border border-amber-200/60 dark:border-amber-800/40 bg-amber-50/40 dark:bg-amber-950/20">
          <SectionHeader icon={AlertTriangle} label="Missing / Not Identified" color="bg-amber-100 dark:bg-amber-900/40 text-amber-600 dark:text-amber-400" />
          <div className="space-y-3">
            {r.missingFields.map((f, i) => (
              <MissingClauseCard key={i} field={f} />
            ))}
          </div>
        </Card>
      )}
