...
- Confirmed on 2026-05-04: `/api/entitlements/native-verify` is a mounted public route that verifies RevenueCat entitlements for a caller-supplied `rcUserId` and then writes subscriber state for a caller-supplied `email`. If RevenueCat API keys are configured in production, this becomes a billing-identity and paywall-integrity boundary that must be audited together with `billingDb.upsertSubscriber()` and the global allowlist behavior.
