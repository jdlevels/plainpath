...
10. **PDF Editor (Slice 3 — complete)**: Full editing workspace at `/app/pdf-editor`. Pro/admin gated. Upload → persistent session → split-screen editor. Left pane = Original (read-only), right pane = Working Copy with overlay ops. Four tools: Select (move/resize/delete), Text (click to place textarea), Mask (drag white rectangle), Highlight (drag yellow semi-transparent rectangle). Undo/redo (Ctrl+Z/Y, history stack, 50 entries). Save system: immediate on draw/place/delete, 800ms debounce on text typing. **Export (Slice 3)**: violet "Download" button in top-right toolbar. On click: flushes pending saves, calls GET `/api/pdf-editor/sessions/:id/export` (Clerk-authed), server uses pdf-lib to load original PDF bytes from DB and bake in all ops (mask → white rect, highlight → yellow rect at 0.4 opacity, text → embedded Helvetica text). Returns modified PDF as `application/pdf` attachment. Client creates object URL and triggers `<a download>` programmatically. Button shows spinner ("Exporting…") during export, resets to idle on success, shows 4s error state on failure. Coordinate conversion: fractional top-left (pdfjs) → points bottom-left (pdf-lib). Session persistence: ops stored as JSONB in PostgreSQL `pdf_editor_sessions`. Key fixes: `Map.prototype.getOrInsertComputed` polyfill for pdfjs-dist v5 on Chromium < 136; ArrayBuffer slice before pdfjs for Strict Mode; Text tool auto-switches to Select after placing. Deferred: page operations, My Docs integration, object storage migration, white-mask editor visibility.
