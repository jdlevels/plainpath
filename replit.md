...
-   **PDF Platform**: Full PDF platform at `/pdf-editor`. Two-tab hub: **Edit PDF** (upload + session list with rename/delete, opens split-screen editor) and **PDF Tools** (stateless utilities). Editor features: Select/Text/Mask/Highlight tools, undo/redo, autosave, export to PDF. PDF type classification badge (Text/Scanned/Mixed) at upload. OCR foundation for scanned/mixed PDFs: amber banner prompts OCR run, OpenAI Vision extracts text per-page (max 20), collapsible OCR review sidebar with inline editing. Session rename (inline pencil icon in toolbar) and delete. PDF Tools panel: Merge (up to 10 files), Extract Pages (ranges), Page Tools (Delete/Rotate/Reorder sub-modes), Compress. DB: `pdf_editor_sessions` with `pdf_type text` and `ocr_data jsonb`. Routes: `/api/pdf-editor/*` (sessions + classify + OCR + rename + delete), `/api/pdf-utilities/*` (merge/extract-pages/page-ops/compress/page-count).
