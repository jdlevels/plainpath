...
10. **PDF Editor (Slice 1 — shell)**: Upload + split-screen workspace shell at `/app/pdf-editor`. Pro/admin gated. Left pane = Original (read-only), right pane = Working Copy baseline. Both panes render the same pdfjs-dist render pass. Editing tools deferred to Slice 2.
