  ...
  const tools = [
    {
      ...
      desc: "Upload any PDF, Word file, or paste text. AI extracts every requirement, deadline, risk, and missing item into a prioritized action plan — with source citations.",
      ...
      <div className="relative flex h-full flex-col px-[8vw] py-[6vh]">
        <div className="mb-[3vh]">
          ...
          <h2 className="text-[3.8vw] font-extrabold leading-[1.0] tracking-tight text-[#f0f4f8] font-display">
            Two tools. Every paperwork problem.
          </h2>
          <p className="mt-[1vh] text-[1.5vw] text-[#f0f4f8]/50 font-body max-w-[60vw]">
            One platform covers the full document lifecycle — from understanding what a form requires to reviewing a contract clause by clause before you sign.
          </p>
        </div>

        <div className="grid grid-cols-2 gap-[2.5vw] flex-1 content-center max-w-[70vw] mx-auto w-full">
          {tools.map((t) => (
            <div key={t.label} className="rounded-[1.3vw] p-[2.2vw] flex flex-col gap-[1.6vh]" style={{ background: t.bg, border: `1px solid ${t.border}` }}>
              <div className="w-[3vw] h-[3vw] rounded-[0.7vw] flex items-center justify-center" style={{ background: `rgba(255,255,255,0.06)` }}>
                {t.icon}
              </div>
              <div className="text-[1.6vw] font-bold font-display" style={{ color: t.tagColor }}>{t.label}</div>
              <p className="text-[1.2vw] text-[#8fa3bc] font-body leading-snug flex-1">{t.desc}</p>
